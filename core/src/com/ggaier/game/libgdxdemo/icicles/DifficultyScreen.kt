...
class DifficultyScreen(val mIciclesGame: IciclesGame) : Screen, InputAdapter() {
    ...
    override fun render(delta: Float) {
        ...
        mFont.draw(mBatch,easyLayout, EASY_CENTER.x, EASY_CENTER.y)

        val mediumLayout=GlyphLayout(mFont, MEDIUM_LABEL)
        mFont.draw(mBatch,mediumLayout, MEDIUM_CENTER.x, MEDIUM_CENTER.y)

        val hardLayout=GlyphLayout(mFont, HARD_LABEL)
        mFont.draw(mBatch, hardLayout, HARD_CENTER.x, HARD_CENTER.y)
        mBatch.end()
